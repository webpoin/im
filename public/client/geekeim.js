...
(function(window, document, undefined){
	...
	var box = (function(window,document, config) {
		...
		// 开发专用，字体图标
		head.push('<link rel="stylesheet" href="/client/style.css" />');
		...
		win.socket = {emit:function(){console.log(111)},on:function(){}}
		...
		doc.enter.onkeypress = function(e) {
				...
				if ( e.keyCode==13||e.keyCode==10){
					...
					val = dom.clearEnter();
					socket.emit('chart messages',val);
					dom.insertChart('customer',val);
					return false;
				}
			}
		...
		// var socket = io.connection('http://localhost:3000');
